...
if (isset($_POST["save"])){
       ...
       include_once("connection.php");
        ...
        $sql = "SELECT * FROM loanuserpayment order by id desc";
	 ...
	 if (isset($_GET["search"])){  
         ...
         $search = $_GET['search'];
         $check_record = "SELECT * FROM loanuserpayment where loanid LIKE '$search' || lastname LIKE '$search' || firstname LIKE '$search' || middlename LIKE '$search' || userid LIKE '$search' || id LIKE '$search' || status LIKE '$search' || loantype LIKE '$search' || gender LIKE '$search' || lasttransaction LIKE '$search' || barangay LIKE '$search' || municipality LIKE '$search' || province LIKE '$search' order by id desc";
    $sqlvalidate = mysqli_query($conn, $check_record);

    if (mysqli_num_rows($sqlvalidate) == 0){
    	echo "<script>alert('No existing data');</script>";
    	echo "<script>window.location.href = './loan_transaction.php'</script>";
    }else{
        $search = $_GET['search'];
...
<body>
	...
	<div class="leftdiv1" id="leftdiv1">
		...
		<div class="btn1" id="btn1">
			...
			<a href="loan_transaction.php" style="background-color: #239cdf;">
...
<!-- 			<img id="menuimg" src="asset/icons/4836503.PNG" width="27" height="27"> -->
		...
		<div class="search-add" id="search-add-refreshbtn">
			...
			<form method="GET" action="loan_transaction.php" autocomplete="off">
				<input type="text" name="search" value="<?php echo $_GET['search'];?>" required>
				<button type="submit">Search</button>
			</form>
			</div>
			<div class="" id="refresh-add">
				<a href="loan_transaction.php" id="refresh"><!-- <img id="menuimg" src="asset/icons/574166.PNG" width="23" height="23"> --><label>Refresh List</label></a>
				<form method="GET" action="./export_loan_transaction.php" id="export">
			...
			<?php 
				        ...
				        if($conn->connect_error){
				            echo $conn->connect_error;
				        	 ...
				        	 $blank = "";
				        	 if($search == $blank){
							$members = "SELECT * FROM loanuserpayment";
							...
							$search = $_GET['search'];
							$members = "SELECT * FROM loanuserpayment where loanid LIKE '$search' || lastname LIKE '$search' || firstname LIKE '$search' || middlename LIKE '$search' || userid LIKE '$search' || id LIKE '$search' || status LIKE '$search' || loantype LIKE '$search' || gender LIKE '$search' || lasttransaction LIKE '$search' || barangay LIKE '$search' || municipality LIKE '$search' || province LIKE '$search' order by id desc";
							$members_run = mysqli_query($conn, $members);
		...
		<div class="table" id="table1">
			...
			<div class="list">
					<div class="tablename" id="tablename" style="width: 20%;">
						<div class="tablelist">
							<label id="label"><?php echo $row['lasttransaction']; ?></label>
						</div>	
					</div>
					<div class="tablename" id="tablename" style="width: 20%;">
						<div class="tablelist">
					...
					<div class="tablename" id="tablename" style="width: 25%;">
						<div class="tablelist">
							<label id="label">₱ <?php echo $row['payment']; ?></label>
						</div>	
					</div>				
					<div class="tablename" id="tablename" style="width: 25%;">
					...
					<div class="tablename" id="tablename" style="width: 30%;">
						<form>
           					<a  href="loan_transaction_print.php?id=<?php echo $row['id'];?>" target="_blank" name=""><img src="asset/icons/6285148.PNG" width="30" height="20"></a>
		...
		<div class="table" id="table2">
			<?php
		        ...
		        $search = $_GET['search'];
         		...
         		$sql = "SELECT * FROM loanuserpayment where lastname LIKE '%$search%' || firstname LIKE '%$search%' || middlename LIKE '%$search%' || userid LIKE '%$search%' || id LIKE '%$search%' || status LIKE '%$search%' || loantype LIKE '%$search%' || gender LIKE '%$search%' || lasttransaction LIKE '%$search%' || loandate LIKE '%$search%' order by id";
                $result = $conn->query($sql) or die ($conn->error);
                $row = $result->fetch_assoc();
			...
			<div class="list">
					...
					<div class="tablename" id="tablename" style="width: 70%;">
						...
						<div class="tablelist">
							<label id="label1">Loan ID :</label ><label id="label2"><?php echo $row['loanid']; ?></label>
						</div>
						<div class="tablelist">
						...
						<div class="tablelist">
							<label id="label1">Total Payable Amount :</label ><label id="label2">₱ <?php echo $row['totalpayableamount']; ?></label>
						</div>
						<div class="tablelist" >
							<label id="label1">Total Balance :</label ><label id="label2">₱ <?php echo $row['totalbalance']; ?></label>
						</div>
						<div class="tablelist">
							<label id="label1">Date of Transaction :</label ><label id="label2"><?php echo $row['lasttransaction']; ?></label>
						</div>
					...
					<div class="tablename" id="tablename" style="width: 30%;">
						<form>
           					<a  href="loan_transaction_print.php?id=<?php echo $row['id'];?>" target="_blank" name=""><img src="asset/icons/6285148.PNG" width="40" height="30"></a>
